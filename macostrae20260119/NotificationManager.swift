...
class NotificationManager {
    ...
    func refreshSubscriptionNotifications() async {
        let granted = await requestAuthorization()
        guard granted else { return }

        do {
            let subscriptions = try await AppwriteService.shared.fetchSubscriptions()
            scheduleNotifications(for: subscriptions)
            checkAndNotifyUpcoming(subscriptions: subscriptions)
        } catch {
            print("❌ Failed to refresh subscription notifications: \(error.localizedDescription)")
        }
    }

    func refreshSubscriptionNotifications(for subscriptions: [Subscription]) async {
        let granted = await requestAuthorization()
        guard granted else { return }
